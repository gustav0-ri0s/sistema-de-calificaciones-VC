...
const DescriptiveCommentModal: React.FC<DescriptiveCommentModalProps> = ({
  ...
  const isEditable = !isLocked && role !== 'Supervisor';
      ...
      <div className="relative bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="p-6">
          ...
          <textarea
            ...
            rows={6}
            className={`w-full p-4 rounded-2xl outline-none focus:ring-2 transition-all text-sm text-gray-700 placeholder:text-gray-300 resize-none ${
              isEditable 
          ...
          {isApproved && role === 'Docente' && (
            <div className="mt-4 flex items-center gap-2 p-3 bg-amber-50 rounded-xl border border-amber-100">
              <ShieldAlert size={16} className="text-amber-500" />
              <p className="text-[11px] text-amber-700 leading-tight">
                <strong>Bloqueado:</strong> El supervisor ya otorgó el visto bueno. No puede modificar este texto sin previa coordinación.
              </p>
            </div>
          )}
